...
std::vector<std::shared_ptr<vsdk::ModelRegistration>> create_all_model_registrations() {
    ...
    registrations.push_back(std::make_shared<vsdk::ModelRegistration>(
        ...
        voice_trigger::VoiceTriggerMic::model,
        [](vsdk::Dependencies deps, vsdk::ResourceConfig config) {
            return std::make_unique<voice_trigger::VoiceTriggerMic>(std::move(deps), std::move(config));
        },
        voice_trigger::VoiceTriggerMic::validate));
...
int main(int argc, char* argv[]) {
    const std::string usage = "usage: voice-trigger-module /path/to/unix/socket";
    if (argc < 2) {
