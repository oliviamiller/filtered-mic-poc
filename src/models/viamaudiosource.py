...
class ViamAudioInSource(AsyncAudioSource):
    ...
    async def _stream_audio_chunks(self):
        ...
        try:
            ...
            # For 16000 Hz, 30ms = 480 samples = 960 bytes (16-bit PCM)
            frame_duration_ms = 30
            frame_size = int(target_sample_rate * frame_duration_ms / 1000) * self._sample_width
            if self.logger:
                self.logger.info(f"Stream: Using {frame_duration_ms}ms frames ({frame_size} bytes) for VAD")

            # Buffer for accumulating audio data
            buffer = bytearray()

            # Get audio stream - this starts the capture (duration=0 for continuous streaming)
            ...
            async for resp in audio_stream:
                ...
                # Yield fixed-size frames for VAD compatibility
